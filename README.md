...
![Public firmware](https://img.shields.io/badge/public-v1.35.3--soft--step-8A2BE2)
![Gen 3 candidate](https://img.shields.io/badge/Gen%203%20RC-v1.36.0-blue)
...
The hosted installer currently serves the stable Gen‑2 firmware
`1.35.3-soft-step` with Pokémon #1–251. Use desktop **Chrome or Edge**:
...
The repository already contains the tested Gen‑3 code and assets for all 386
Pokémon, but the hosted installer has **not** been switched to Gen 3 yet.

- Local debug build: `1.36.0-gen3-local` through `web/dev.html`.
- Public release candidate: `1.36.0` through `web/release-gen3.html`.
- The release candidate contains no local Pokémon/evolution/battle test commands.
- Neither page replaces the hosted Gen‑2 installer until Gen 3 is explicitly approved.
...
```bash
bash tools/build_web.sh --check         # pinned public Gen 2, 251 Pokémon
bash tools/build_web_local.sh --check   # Gen 3 debug build, 386 Pokémon
bash tools/build_web_gen3.sh --check    # public Gen 3 RC, no debug commands
```
...
- `http://127.0.0.1:8000/` — unchanged public-like Gen‑2 installer
- `http://127.0.0.1:8000/dev.html` — Gen‑3 hardware/debug installer
- `http://127.0.0.1:8000/release-gen3.html` — public Gen‑3 release preview
...
bash tools/build_web_gen3.sh --check
git diff --check
```
